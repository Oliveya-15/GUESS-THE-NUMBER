    ...
    def startgame(range, attempt, max_score):
        def result(userno, attempt, max_score):
            ...
            if guess == generated:
                ...
                print(duration_str) # Extract hours, minutes, and seconds
                rangelabel.destroy()
                userno.destroy()
                submit.destroy()
                hintb.destroy()
                calculate_score(max_score)
                cong = Label(root, text=f'''🥳🥳🥳Congratulations {player_name},\n You Won The Game🥳🥳🥳  \n Your Score: {score} \n Time taken: {duration_str}''', font=("times new roman", 24, "bold"), bg="black",fg="white", padx=10, pady=10)
                cong.pack(pady=20)
                threading.Thread(target=speak, args=(f"Congratulations {player_name}, You Won The Game \n Your Score: {score}. Time taken: {duration_str}",)).start()
...
image = Image.open("C:/Users/HP/OneDrive/Desktop/GTN FOLDER/gtn logo.jpg")
